...
class GudangController extends Controller {
		...
		try {
			if (!$type){
				if ($postheader){
					...
					$id = TransaksiGudang::saveTransaksi($action, $header, $kontainer, $detail);
				}
				else {
					$id = $request->input("delete");
					if ($id && $id != ""){
						TransaksiGudang::deleteTransaksi($id);
					}
				}
			}
			else if ($type == "kontainermasuk"){
				if ($request->input("delete")){
						$id = $request->input("delete");
						if ($id && $id != ""){
							TransaksiGudang::deleteTransaksiBayar($id);
						}
				}
				else {
						$id = TransaksiGudang::saveKontainerMasuk($request->input());
				}
			}
	...
	public function banding(Request $request)
  {
	...
	public function perekamanbongkar(Request $request)
  {
		if(!auth()->user()->can('gudang.bongkar')){
			abort(403, 'User does not have the right roles.');
		}
		...
		if ($filter && $filter == "1"){
			...
			$breadcrumb[] = Array("link" => "../", "text" => "Home");
			$breadcrumb[] = Array("text" => "Browse Bongkar");
			$importir = Transaksi::getImportir();
	...
	public function kontainermasuk(Request $request)
	{
			if(!auth()->user()->can('gudang.kontainermasuk')){
				abort(403, 'User does not have the right roles.');
			}
			if ($request->ajax()){
					if (isset($request->nokontainer)){
							...
							if ($data->exists()){
									...
									return response()->json(["error" => "Nomor Kontainer tidak ada"]);
							}
					}
			}
			else {
					$breadcrumb[] = Array("link" => "/", "text" => "Home");
					$breadcrumb[] = Array("text" => "Kontainer Masuk");
					$gudang = Gudang::all();
