    ...
    <script>
        $(function(){
            var table = $("#grid").DataTable({
                ...
                {
                    $(row).attr("row-id", data.KODEACC_ID);
                },
                buttons: [{
                text: 'Tambah',
                ...
                action: function (e, dt){
                    ...
                    $("#modal .btn-ok").html("Ya").on("click", function(){
                        ...
                        $.ajax({
                            url: "/master/crud",
                            data: {_token: "{{ csrf_token() }}", action: "kodeacc", input: $.param({"input-action": "delete", "id": row[0].KODEACC_ID})},
                            type: "POST",
            ...
            $("#saveform").on("click", function(){
                ...
                $.ajax({
                    url: "/master/crud",
                    data: {_token: "{{ csrf_token() }}", action: "kodeacc", input: $("#form").serialize()},
                    type: "POST",
                    success: function(msg) {
                        if (typeof msg.error != 'undefined'){
                            $("#modal .modal-body").html(msg.error);
